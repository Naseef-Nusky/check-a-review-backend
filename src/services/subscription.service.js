...
export const subscriptionService = {
  ...
  async createCheckout(businessId, userId, plan) {
    ...
    let customerId = sub.square_customer_id || null
  ...
  async handleWebhook(event) {
    ...
    if (type === 'subscription.created' || type === 'subscription.updated') {
      ...
      const plan = note?.plan && ['starter', 'premium'].includes(note.plan) ? note.plan : null
    ...
    if (type === 'payment.updated' || type === 'payment.created') {
      ...
      if (note?.plan && ['starter', 'premium'].includes(note.plan)) {
        await query(
          `UPDATE subscriptions
...
async function inferPendingPlan(customerId) {
  ...
  if (pending && ['starter', 'premium'].includes(pending)) return pending
